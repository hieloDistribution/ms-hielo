 ...
 * Mirrors the order-service-side counterpart.
 */
class InternalSecurityConfigFilterTest {

    private static final String TOKEN = "expected-token-xyz";
    ...
    void setUp() {
        filter = new InternalSecurityConfig.BearerServiceTokenFilter(TOKEN,
                List.of(new org.springframework.security.core.authority.SimpleGrantedAuthority("SCOPE_internal:read")));
    ...
    void noHeader_rejectsWith401() throws Exception {
        MockHttpServletRequest req = new MockHttpServletRequest("GET", "/internal/auth/users/abc");
